...
解压 `release/resume-application-copilot-v0.3.1.zip`，然后在扩展管理页选择解压后的文件夹。
浏览器扩展不能直接从 ZIP 中加载。

Extract `release/resume-application-copilot-v0.3.1.zip`, then select the extracted folder from the extension manager. Browsers cannot load the extension directly from a ZIP file.
